...
public class Parser {
	...
	public static Document parse(String filepath) throws ParserException,
		...
		try {
			...
			// modifiedArticle = modifiedArticle.replace(title, "").trim(); //
			// title
				...
				if (line != null
					...
					if (line.contains("<AUTHOR>")) {
						...
						if (!line.contains("</AUTHOR>")) {
							...
							authorLine = modifiedArticle.substring(
									modifiedArticle.indexOf("<AUTHOR>"),
									modifiedArticle.indexOf("</AUTHOR>") + 9);
						}

						// contains authors and organization
						String authorsAndOrg[] = authorLine
								.replace("<AUTHOR>", "")
						...
						// Authors
						String[] Authors = authorsAndOrg[0].trim().split(
								" and ");
						...
						modifiedArticle = modifiedArticle.replace(authorLine,
								"").trim(); // author
						// line
						// removed
						continue;
						...
						if (m.find()) {
							...
							ArticlePlace = datePlace[0].substring(0,
									datePlace[0].lastIndexOf(","));
							documentObj.setField(FieldNames.PLACE,
									ArticlePlace.trim());

							// anything after - is content
							Content = modifiedArticle.substring(modifiedArticle
									.indexOf('-') + 1);
